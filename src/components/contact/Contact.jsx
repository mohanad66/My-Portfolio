                        ...
                        <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4" aria-label="Contact form to send a project inquiry">
                            ...
                            {status.message && (
                                <motion.div
                                    ...
                                    className={`p-3 md:p-4 rounded-xl md:rounded-2xl text-center font-semibold text-sm ${status.success ? 'bg-emerald-500/10 border border-emerald-500/50 text-emerald-400' : 'bg-red-500/10 border border-red-500/50 text-red-400'}`}
                                >
                                    {status.message}
                                </motion.div>
                            )}
                            ...
                            <motion.button
                                ...
                                whileHover={{ scale: status.loading || mobile ? 1 : 1.05 }}
                                whileTap={{ scale: status.loading || mobile ? 1 : 0.95 }}
                                className="w-full py-4 md:py-5 bg-emerald-600 text-black font-black uppercase tracking-[0.15em] md:tracking-[0.2em] rounded-xl md:rounded-2xl hover:bg-emerald-400 transition-all shadow-2xl shadow-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 text-xs md:text-sm"
                            >
                                {status.loading ? (
                                    <>
